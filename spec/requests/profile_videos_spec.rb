...
RSpec.describe 'Profile videos API' do
  ...
  let!(:medium_type) { create(:medium_type, code: 'VID')}
  ...
  let!(:profile_video1) { create(:profile_video, profile_id: profile.id,
    artist_medium_id: artist_medium1.id, sequence: 1, is_primary: true)}
  let!(:profile_video2) { create(:profile_video, profile_id: profile.id,
    artist_medium_id: artist_medium2.id, sequence: 3, is_primary: false)}
  let!(:profile_video3) { create(:profile_video, profile_id: profile.id,
    artist_medium_id: artist_medium3.id, sequence: 2, is_primary: false)}
  let(:profile_id) { profile.id }
  let(:profile_video_id) { profile_video2.id}
  let(:primary_profile_video_id) { profile_video1.id}

  describe 'GET /profiles/:profile_id/videos' do
    before { get "/profiles/#{profile_id}/videos"}
    ...
    it 'return videos' do
      ...
      expect(json[0]['id']).to eq(profile_video1.id)
      expect(json[1]['id']).to eq(profile_video3.id)
      expect(json[2]['id']).to eq(profile_video2.id)
    end
  end

  describe 'GET /profiles/:profile_id/videos/preview' do
    before { get "/profiles/#{profile_id}/videos/preview"}
  ...
  describe 'PUT /profiles/:profile_id/videos/:id/primary' do
    before { put "/profiles/#{profile_id}/videos/#{profile_video_id}/primary"}
    ...
    it 'change primary correct' do
       get "/profiles/#{profile_id}/videos"
       expect(json[0]['is_primary']).to eq(false)
       expect(json[2]['is_primary']).to eq(true)
    end
  end

  describe 'DELETE /profiles/:profile_id/videos/:id' do
    before { delete "/profiles/#{profile_id}/videos/#{primary_profile_video_id}"}

    it 'return status code 204' do
      expect(response).to have_http_status(204)
    end

    it 'delete profile video correctly' do
       get "/profiles/#{profile_id}/videos/preview"
       expect(json.size).to eq(3)
    end

    it 'change primary correctly when delete primary video' do
       get "/profiles/#{profile_id}/videos"
       expect(json.size).to eq(2)
    end
  end

  describe 'PUT /profiles/:profile_id/videos/apply' do
    let(:valid_params) {{
        artist_medium_id: artist_medium4.id
    }}
    before { put "/profiles/#{profile_id}/videos/apply", params: valid_params}

    it 'return status code 201' do
      expect(response).to have_http_status(201)
    end

    it 'return profile video' do
  ...
  describe 'PUT /profiles/:profile_id/videos/resort' do
    let(:valid_params) {{
        from_profile_video_id: profile_video1.id,
        to_profile_video_id: profile_video3.id
    }}

    before { put "/profiles/#{profile_id}/videos/resort", params: valid_params}

    it 'return status code 204' do
      expect(response).to have_http_status(204)
    end

    it 'resort video sequence' do
      get "/profiles/#{profile_id}/videos"
      expect(json[0]['id']).to eq(profile_video3.id)
    end
  end
end
